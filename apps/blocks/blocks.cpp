...
const int kCanvasHeight =
    kGapHeight + kNumBlocksY * kBlockHeight +
    kGapBar + kBarHeight + kGapHeight + kBarFloat;
...
void DrawBar(uint64_t layer_id, int bar_x)
{
    SyscallWinFillRectangle(layer_id,
                            4 + bar_x, 24 + kBarY,
                            kBarWidth, kBarHeight, 0xff'ff'ff);
}
...
void DrawBall(uint64_t layer_id, int x, int y)
{
    SyscallWinFillRectangle(layer_id,
                            ...
                            2 * kBallRadius, 2 * kBallRadius, 0x00'7f'00);
    SyscallWinFillRectangle(layer_id,
                            4 + x - kBallRadius / 2, 24 + y - kBallRadius / 2,
                            kBallRadius, kBallRadius, 0x00'ff'00);
}
...
extern "C" void main(int argc, char **argv)
{
    auto [layer_id, err_openwin] =
        SyscallOpenWindow(kCanvasWidth + 8, kCanvasHeight + 28,
                          10, 10, "blocks");
    if (err_openwin)
    {
        exit(err_openwin);
    ...
    {
        blocks[y].set(); // set 1 for all bits
    }
    ...
    for (;;)
    {
        ...
        do
        {
            ...
            if ((ball_x_ < block_left && block_left <= ball_x_) ||
                (block_right < ball_x && ball_x_ <= block_right))
            {
                ball_dir = 180 - ball_dir;
            }
            if ((ball_y_ < block_top && block_top <= ball_y_) ||
