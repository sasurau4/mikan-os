             ...
             int mouse_x, int mouse_y, uint32_t color)
{
    ...
    SyscallWinFillRectangle(layer_id_flags, eye_x - kEyeSize / 2, eye_y - kEyeSize / 2,
                            kEyeSize, kEyeSize, color);
}
...
extern "C" void main(int argc, char **argv)
{
    ...
    SyscallWinFillRectangle(layer_id, 4, 24, kCanvasSize, kCanvasSize, 0xff'ff'ff);
    ...
    while (true)
    {
        ...
        else if (events[0].type == AppEvent::kMouseMove)
        {
            ...
            SyscallWinFillRectangle(layer_id | LAYER_NO_REDRAW, 4, 24,
                                    kCanvasSize, kCanvasSize, 0xff'ff'ff);
            DrawEye(layer_id, arg.x, arg.y, 0x00'00'00);
        }
        else
        {
