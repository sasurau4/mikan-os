...
uint64_t OpenTextWindow(int w, int h, const char *title)
{
    ...
    fill_rect(3, 23, 1 + 8 * w, 1, 0x66'66'66);
    fill_rect(3, 24, 1, 1 + 16 * h, 0x66'66'66);
    fill_rect(4, 25 + 16 * h, 1 + 8 * w, 1, 0xcc'cc'cc);
    fill_rect(5 + 8 * w, 24, 1, 1 + 16 * h, 0xcc'cc'cc);
...
{
    char buf[1024];
    SyscallWinFillRectangle(layer_id, 4, 24, 8 * w, 16 * h, 0xff'ff'ff);
    ...
    for (int i = 0; i < h; ++i)
    {
        ...
        SyscallWinWriteString(layer_id, 4, 24 + 16 * i, 0x00'00'00, buf);
    }
}
