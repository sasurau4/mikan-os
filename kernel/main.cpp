...
/**
 * WritePixel function writes a pixel to the frame buffer.
 * @retval 0 on success
 * @retval non 0 on failure
 */
...
KernelMain(const FrameBufferConfig &frame_buffer_config)
{
    for (int x = 0; x < frame_buffer_config.horizontal_resolution; ++x)
    {
        ...
        {
            WritePixel(
                frame_buffer_config, x, y, {255, 255, 255});
        }
    }
    for (int x = 0; x < 200; ++x)
    {
        ...
        {
            WritePixel(
                frame_buffer_config, 100 + x, 100 + y, {0, 255, 0});
        }
    }
    while (1)
        __asm__("hlt");
