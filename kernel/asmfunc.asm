...
global IoOut32 ; void IoOut32(uint16_t port, uint32_t data);
IoOut32:
    mov dx, di      ; dx = addr
...
global IoIn32 ; uint32_t IoIn32(uint16_t port);
IoIn32:
    mov dx, di      ; dx = addr
    in eax, dx
    ret
...
CallApp: ; void CallApp(int argc, char **argv, uint16_t ss, 
    ...
    push rcx ; RIP
    o64 retf
    ; Not reached to this line when tha app is finished
...
SyscallEntry:
    ...
    ; rbx, r12-r15 are callee-saved, so they are not used for passing arguments or return value
    ; rax is used for return value, it is not used for passing arguments

    mov rsp, rbp

    pop rsi ; Restore original syscall number
    cmp esi, 0x80000002
    je .exit
